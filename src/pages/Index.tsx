  ...
  return (
    <div className="min-h-screen bg-white text-slate-950">
      ...
      <main>
          ...
          <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[0.82fr_1.18fr]">
            <div className="max-w-xl">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <BotaoDeComprar rotulo={copy.hero.primary} className="bg-[#2457D6] hover:bg-[#1d47b0]"><ArrowRight className="ml-2 h-4 w-4" /></BotaoDeComprar>
        ...
        <section id="pricing" className="scroll-mt-20 px-4 py-20 sm:px-6"><div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-8 rounded-3xl border border-violet-200 bg-violet-50/60 p-8 md:flex-row md:p-10"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#6D43C0]">{copy.pricing.eyebrow}</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950">{copy.pricing.title}</h2><p className="mt-3 max-w-xl text-lg leading-7 text-slate-600">{copy.pricing.body}</p></div><div className="w-full max-w-64 text-center md:text-right"><p className="text-sm font-medium text-slate-600">{copy.pricing.availableLabel}</p><p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-[#6D43C0]">{copy.pricing.promoLabel}</p><p className="mt-2 flex items-baseline justify-center gap-2 md:justify-end"><span className="text-lg font-medium text-slate-400 line-through">R${PRECO_REGULAR_BRL}</span><span className="text-5xl font-bold text-slate-950">R${PRECO_PROMO_BRL}</span><span className="text-slate-500">{copy.pricing.monthly}</span></p><p className="mt-2 text-xs leading-5 text-slate-500">{comVagas(copy.pricing.promoNote)}</p><p className="mt-2 text-xs leading-5 text-slate-500">{copy.pricing.availableHint}</p><BotaoDeComprar rotulo={copy.pricing.cta} className="mt-5 w-full bg-[#2457D6] hover:bg-[#1d47b0]" /></div></div></section>
      </main>
