...
const THEMES: Theme[] = [
  {
    ...
    noun: { pt: 'a limpeza', es: 'la limpieza', en: 'cleanliness' },
    fix: {
    ...
    praise: { pt: 'tenha reparado no cuidado com a limpeza', es: 'hayas notado el cuidado con la limpieza', en: 'you noticed how much care we put into keeping the place clean' },
  },
  {
    id: 'atendimento',
    ...
    noun: { pt: 'o atendimento', es: 'la atención', en: 'the service you received' },
    fix: {
    ...
    praise: { pt: 'o tenhamos atendido como merece', es: 'te hayamos atendido como mereces', en: 'we looked after you the way we should' },
  },
  {
    id: 'espera',
    ...
    noun: { pt: 'o tempo de espera', es: 'el tiempo de espera', en: 'the wait' },
    fix: {
    ...
    praise: { pt: 'tenha sido servido sem esperas', es: 'te hayamos servido sin esperas', en: 'we got everything to you without a wait' },
  },
  {
    id: 'comida',
    ...
    noun: { pt: 'a comida', es: 'la comida', en: 'the food' },
    fix: {
    ...
    praise: { pt: 'a comida tenha estado à altura', es: 'la comida haya estado a la altura', en: 'the food lived up to what you hoped for' },
  },
  {
    id: 'pedido',
    ...
    noun: { pt: 'o seu pedido', es: 'tu pedido', en: 'your order' },
    fix: {
    ...
    praise: { pt: 'esteja tudo certo com o seu pedido', es: 'todo haya salido bien con tu pedido', en: 'everything came out right with your order' },
  },
  {
    id: 'preco',
    ...
    noun: { pt: 'a relação entre o que pagou e o que recebeu', es: 'la relación entre lo que pagaste y lo que recibiste', en: 'what you got for what you paid' },
    fix: {
    ...
    praise: { pt: 'tenha sentido que valeu o que pagou', es: 'hayas sentido que valió lo que pagaste', en: 'you felt it was worth what you paid' },
  },
  {
    id: 'ambiente',
    ...
    noun: { pt: 'o ambiente da sala', es: 'el ambiente del local', en: 'the atmosphere in the room' },
    fix: {
    ...
    praise: { pt: 'se tenha sentido bem na sala', es: 'te hayas sentido a gusto en el local', en: 'the room felt right to you' },
  },
];
...
const GENERIC = {
  noun: { pt: 'a sua experiência', es: 'tu experiencia', en: 'your experience' },
  fix: {
  ...
  praise: {
    pt: 'tenha gostado da visita', es: 'te haya gustado la visita', en: 'you enjoyed your visit',
  },
} as const;

const buildGreeting = (locale: ReplyLocale, name?: string | null): string => {
  const clean = name?.trim();
  if (!clean || clean.toLowerCase().includes('anon')) {
    return { pt: 'Olá,', es: 'Hola,', en: 'Hello,' }[locale];
  }
...
interface Variant {
  id: string;
  title: Record<ReplyLocale, string>;
  hint: Record<ReplyLocale, string>;
  body: Record<ReplyLocale, Builder>;
}
...
const PUBLIC_NEGATIVE: Variant[] = [
  {
    id: 'curta',
    title: { pt: 'Curta e directa', es: 'Corta y directa', en: 'Short and direct' },
    hint: {
  ...
  {
    id: 'com-accao',
    title: { pt: 'Com o que já foi feito', es: 'Con lo que ya se hizo', en: 'With what you already did' },
    hint: {
...
const PUBLIC_NEUTRAL: Variant[] = [
  {
    id: 'perguntar',
    title: { pt: 'Perguntar o que faltou', es: 'Preguntar qué faltó', en: 'Ask what was missing' },
    hint: {
...
const PUBLIC_POSITIVE: Variant[] = [
  {
    id: 'agradecer',
    title: { pt: 'Agradecer com especificidade', es: 'Agradecer con detalle', en: 'Thank them specifically' },
    hint: {
...
const PRIVATE_NEGATIVE: Variant[] = [
  {
    ...
    title: { pt: 'Contacto no próprio dia', es: 'Contacto el mismo día', en: 'Same-day contact' },
    hint: {
  ...
  {
    id: 'com-reparacao',
    title: { pt: 'Com uma reparação', es: 'Con una compensación', en: 'With something to make up for it' },
    hint: {
...
const PRIVATE_POSITIVE: Variant[] = [
  {
    ...
    title: { pt: 'Agradecer e convidar', es: 'Agradecer e invitar', en: 'Thank and invite' },
    hint: {
...
export const buildReplySuggestions = (input: ReplySuggestionInput): ReplySuggestion[] => {
  ...
  const ctx: Ctx = {
    greeting: buildGreeting(locale, input.customerName),
    noun: theme ? theme.noun[locale] : GENERIC.noun[locale],
    fix: theme ? theme.fix[locale] : GENERIC.fix[locale],
    praise: theme ? theme.praise[locale] : GENERIC.praise[locale],
    signature: buildSignature(locale, input.businessName),
  };

  return VARIANTS[input.channel][sentiment].map((variant) => ({
    id: variant.id,
    title: variant.title[locale],
    hint: variant.hint[locale],
    body: variant.body[locale](ctx),
  }));
};
