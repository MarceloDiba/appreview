  ...
  return <div className="space-y-5">
    ...
    {tab === 'whatsapp' ? <FullWhatsAppWorkspace snapshot={snapshot} localWhatsApp={whatsApp} /> : tab === 'reviews' ? <ResponseQueue reviews={queue} snapshot={snapshot} /> : <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
      <section className="min-w-0 space-y-5">
        ...
        <VolumeCard weeks={history} snapshot={snapshot} />
...
const VolumeCard = ({ weeks, snapshot }: { weeks: Week[]; snapshot: ExperimentalApifySnapshot }) => {
  ...
  return <Card className="border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]"><CardContent className="p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-slate-950">Volume de avaliações</h2><span className="text-sm text-slate-500">{hasHistory ? '12 semanas' : 'leitura atual'}</span></div>{hasHistory ? <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center"><div className="h-12 w-40 shrink-0"><ResponsiveContainer width="100%" height="100%"><LineChart data={weeks}><Line type="monotone" dataKey="reviewCount" stroke="#2457D6" strokeWidth={3} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div><p className="text-lg font-semibold text-slate-950">{current.reviewCount} <span className="text-sm font-normal text-slate-600">avaliações nesta semana · média de {Math.round(average)}</span></p></div> : <p className="mt-5 text-lg font-semibold text-slate-950">{snapshot.sample.reviewCount} <span className="text-sm font-normal text-slate-600">avaliações lidas na amostra</span></p>}{change !== null && change <= -25 && <div className="mt-5 flex gap-3 rounded-lg border border-red-100 bg-red-50 p-4 text-sm leading-5 text-red-950"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-red-700" /><p><strong>Queda de {Math.abs(change)}%</strong> em relação à média das últimas 8 semanas.</p></div>}</CardContent></Card>;
};
...
const ProfileCompleteness = ({ connected }: { connected: boolean }) => {
  const { t } = useOwnerTranslation();
  return <Card className="border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]"><CardContent className="p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-slate-950">{t('dashboard.cockpit.layout.profileTitle')}</h2>{connected && <span className="text-sm text-slate-500">—</span>}</div><p className="mt-4 text-sm leading-5 text-slate-600">{t('dashboard.cockpit.layout.profileBody')}</p><Button asChild variant="link" className="mt-2 h-auto px-0 text-[#2457D6]"><Link to="/settings">{t('dashboard.cockpit.layout.googleAction')}<ChevronRight className="ml-1 h-4 w-4" /></Link></Button></CardContent></Card>;
};

const WeeklyChange = ({ weeks }: { weeks: Week[] }) => {
  const { t } = useOwnerTranslation();
  const current = weeks.at(-1)?.ownerReplies || 0;
  return <Card className="border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]"><CardContent className="p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-slate-950">{t('dashboard.cockpit.layout.changeTitle')}</h2><span className="text-xs text-slate-500">7 dias</span></div>{weeks.length ? <div className="mt-4 flex items-center gap-3"><div className="h-8 w-20"><ResponsiveContainer width="100%" height="100%"><LineChart data={weeks}><Line type="monotone" dataKey="ownerReplies" stroke="#2457D6" strokeWidth={2.5} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div><p className="text-sm leading-5 text-slate-600">{current ? `Você respondeu ${current} avaliações nos últimos 7 dias.` : '—'}</p></div> : <p className="mt-4 text-sm font-medium text-slate-700">{t('dashboard.cockpit.layout.firstReading')}</p>}</CardContent></Card>;
};
