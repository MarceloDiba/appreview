    ...
    <Card className="rounded-xl border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]">
      <CardContent className="grid gap-4 p-5 md:grid-cols-[1.15fr_repeat(3,1fr)] md:items-center">
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-slate-500">{t('dashboard.googleOutcome.pathDisclaimer')}</p>
        </div>
        <div className="flex items-center gap-3 md:border-l md:border-slate-200 md:pl-5">
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-50"><QrCode className="h-4 w-4 text-[#2457D6]" /></span>
          <div><p className="text-2xl font-semibold text-slate-950">{integer.format(data.qrOpens)}</p><p className="text-xs text-slate-500">{t('dashboard.googleOutcome.qrOpens')}</p></div>
        </div>
        <div className="flex items-center gap-3 md:border-l md:border-slate-200 md:pl-5">
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-50"><MousePointer2 className="h-4 w-4 text-[#2457D6]" /></span>
          <div><p className="text-2xl font-semibold text-slate-950">{integer.format(data.googleClicks)}</p><p className="text-xs text-slate-500">{t('dashboard.googleOutcome.googleClicks')}</p></div>
        </div>
        <div className="flex items-center gap-3 md:border-l md:border-slate-200 md:pl-5">
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-50"><TrendingUp className="h-4 w-4 text-emerald-700" /></span>
          <div><p className="text-2xl font-semibold text-emerald-700">{data.clickThroughRate === null ? '—' : `${percent.format(data.clickThroughRate)}%`}</p><p className="text-xs text-slate-500">{t('dashboard.googleOutcome.continuation')}</p></div>
        </div>
        <Button asChild variant="link" className="h-auto justify-start p-0 text-[#2457D6] md:col-start-4 md:justify-end">
        ...
        <CardContent className="p-0">
          <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-lg font-semibold text-slate-950">{t('dashboard.googleOutcome.summaryTitle')}</h1>
                {illustrative && <ExampleBadge />}
              </div>
              <p className="mt-1 text-sm text-slate-500">{t('dashboard.googleOutcome.summarySubtitle')}</p>
            </div>
          ...
          <div className="grid lg:grid-cols-[0.85fr_1fr_1.55fr]">
            <div className="border-b border-slate-200 p-5 lg:border-b-0 lg:border-r">
              ...
              <p className="mt-3 text-4xl font-medium tracking-tight text-slate-950">{decimal.format(data.averageRating)}</p>
            ...
            <div className="border-b border-slate-200 p-5 lg:border-b-0 lg:border-r">
              <div className="flex items-center gap-2 text-sm font-medium text-slate-600">
                <MessageSquareText className="h-5 w-5 text-[#2457D6]" aria-hidden="true" />
                {t('dashboard.googleOutcome.totalReviews')}
              </div>
              <p className="mt-3 text-4xl font-medium tracking-tight text-slate-950">{integer.format(data.totalReviews)}</p>
              <p className="mt-2 text-sm text-emerald-700">
            ...
            <div className="min-h-52 p-5">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-slate-900">{t('dashboard.googleOutcome.chartTitle')}</p>
              </div>
              {chartData.length >= 2 ? (
                <div className="mt-3 h-32" aria-label={t('dashboard.googleOutcome.chartAria')}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                      ...
                      <Line type="monotone" dataKey="averageRating" stroke="#2457D6" strokeWidth={3} dot={false} activeDot={{ r: 5 }} isAnimationActive={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              ) : (
                <div className="mt-3 flex h-32 items-center justify-center rounded-lg bg-slate-50 px-6 text-center text-sm text-slate-500">
          ...
          <div className="flex flex-col gap-2 border-t border-slate-200 px-5 py-3.5 text-sm text-slate-600 md:flex-row md:items-center">
            <Info className="h-4 w-4 shrink-0 text-[#2457D6]" aria-hidden="true" />
            <strong className="text-slate-900">{t('dashboard.googleOutcome.advisorReading')}</strong>
