...
const LegacyExperimentalCockpitDashboard = ({ snapshot, userId }: { snapshot: ExperimentalApifySnapshot; userId?: string }) => {
...
const DailyPractice = ({ snapshot, onOpenQueue, compact = false }: { snapshot: ExperimentalApifySnapshot; onOpenQueue: () => void; compact?: boolean }) => {
  ...
  const action = practice === 'reply'
    ...
    : practice === 'photos' && snapshot.business.googleReviewUrl
      ...
      : practice === 'invite'
        ? <Button asChild variant="outline" className="rounded-full"><Link to="/qrcodes">{copy.action}<ChevronRight className="ml-1 h-4 w-4" /></Link></Button>
...
export const WhatsAppWorkspace = ({ localWhatsApp, snapshot }: { localWhatsApp: LocalWhatsAppState; snapshot: ExperimentalApifySnapshot }) => {
  const { t, i18n } = useOwnerTranslation();
