...
const migrateLegacyBrazilPhone = (value: string) => {
  const digits = value.replace(/\D/g, '');
  return !value.trim().startsWith('+') && (digits.length === 10 || digits.length === 11) ? `+55${digits}` : value;
};
...
export const WhatsAppNotificationWorkspace = ({ localWhatsApp, onboardingPhone, demoPhone, demo = false }: { localWhatsApp: LocalWhatsAppState; onboardingPhone?: string; demoPhone?: string; demo?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const recipient = migrateLegacyBrazilPhone(stored.recipient || onboardingPhone || '');
    setPreferences({ ...stored, recipient });
    if (onboardingPhone) setTestRecipient((current) => current || migrateLegacyBrazilPhone(onboardingPhone));
  ...
  const refreshDeliveryState = async () => {
    ...
    if (state.preferences) setPreferences({ ...state.preferences, recipient: migrateLegacyBrazilPhone(state.preferences.recipient) });
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
    <section className="space-y-5">
      <Card className="border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]"><CardContent className="p-6">
        ...
        <div className="mt-4 space-y-3">{choices.map((choice) => <label key={choice.key} className="flex items-start gap-3 rounded-xl border border-slate-200 p-4 text-sm leading-5 text-slate-700"><Checkbox checked={preferences[choice.key]} onCheckedChange={(checked) => setChoice(choice.key, checked === true)} /><span><strong className="block text-slate-950">{choice.title}</strong>{choice.body}</span></label>)}</div>
        <div className="mt-5 grid gap-4 sm:grid-cols-3"><label className="text-sm font-medium text-slate-700 sm:col-span-2">{t('whatsappPilot.notificationRecipient')}<div className="mt-2"><InternationalPhoneField id="whatsapp-notification-recipient" value={preferences.recipient} onChange={(value) => setPreferences((current) => ({ ...current, recipient: value }))} placeholder={t('whatsappPilot.phonePlaceholder')} ariaLabel={t('whatsappPilot.phoneCountryLabel')} /></div><span className="mt-1 block text-xs font-normal leading-5 text-slate-500">{onboardingPhone ? t('whatsappPilot.onboardingPhoneHint') : t('whatsappPilot.notificationRecipientHint')}</span></label><label className="text-sm font-medium text-slate-700">{t('dashboard.cockpit.whatsapp.time')}<Input type="time" value={preferences.time} onChange={(event) => setPreferences((current) => ({ ...current, time: event.target.value }))} className="mt-2" /></label></div>
      ...
      <Card className="border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]"><CardContent className="p-6">
        ...
        <div className="mt-6 space-y-4">
          <label className="block text-sm font-medium text-slate-700">{t('whatsappPilot.testRecipient')}<div className="mt-2"><InternationalPhoneField id="whatsapp-test-recipient" value={testRecipient} onChange={setTestRecipient} placeholder={t('whatsappPilot.phonePlaceholder')} ariaLabel={t('whatsappPilot.phoneCountryLabel')} disabled={!ready || sendState.status === 'sending'} /></div><span className="mt-1 block text-xs font-normal leading-5 text-slate-500">{t('whatsappPilot.testRecipientHint')}</span></label>
          <label className="block text-sm font-medium text-slate-700">{t('dashboard.cockpit.whatsapp.message')}<Textarea value={message} onChange={(event) => setMessage(event.target.value)} className="mt-2 min-h-28 resize-y" disabled={!ready || sendState.status === 'sending'} /><span className="mt-1 block text-xs font-normal leading-5 text-slate-500">{t('whatsappPilot.testMessageHint')}</span></label><label className="flex items-start gap-3 rounded-xl border border-slate-200 p-3 text-sm leading-5 text-slate-700"><Checkbox checked={confirmed} onCheckedChange={(checked) => setConfirmed(checked === true)} disabled={!ready || sendState.status === 'sending'} /><span>{t('dashboard.cockpit.whatsapp.confirmation')}</span></label>
